...
def initialize_clients():
    try:
        ...
        # If .env not found or keys missing, try Streamlit secrets (deployment)
        ...
        if not all([cohere_api_key, gemini_api_key, tavily_api_key, groq_api_key]):
            st.error("API keys not found. Please configure .env file (local) or Streamlit secrets (deployment)")
            st.stop()

        co = cohere.Client(api_key=cohere_api_key, timeout=30)
        # Initialize google.genai client instead of configure
        gemini_client = genai.Client(api_key=gemini_api_key)
        tavily = TavilyClient(api_key=tavily_api_key)
        groq = Groq(api_key=groq_api_key, timeout=30)
        
        return co, gemini_client, tavily, groq
